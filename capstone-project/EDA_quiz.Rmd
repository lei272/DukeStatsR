...
1. Which of the following are the three variables with the highest number of missing observations? [B]
<ol>
...
sort(colSums(is.na(ames_train)), decreasing=T)
```
...
2. How many categorical variables are coded in R as having type `int`?  Change them to factors when conducting your analysis. [A]
<ol>
<li> 0
<li> 1
<li> 2
...
3. In terms of price, which neighborhood has the highest standard deviation? [A]
<ol>
<li> `StoneBr`
<li> `Timber`
<li> `Veenker`
...
4. Using scatter plots or other graphical displays, which of the following variables appears to be the best single predictor of `price`? [C]
<ol>
<li> `Lot.Area`
<li> `Bedroom.AbvGr`
<li> `Overall.Qual`
...
ggplot(ames_train, aes(Lot.Area, price)) + geom_point()
ggplot(ames_train, aes(Bedroom.AbvGr, price)) + geom_point()
ggplot(ames_train, aes(Overall.Qual, price)) + geom_point()
ggplot(ames_train, aes(Year.Built, price)) + geom_point()
```
...
5. Suppose you are examining the relationship between `price` and `area`.  Which of the following variable transformations makes the relationship appear to be the most linear? [D]
<ol>
...
ggplot(ames_train, aes(area, price)) + geom_point()
ggplot(ames_train, aes(area, log(price))) + geom_point()
ggplot(ames_train, aes(log(area), price)) + geom_point()
ggplot(ames_train, aes(log(area), log(price))) + geom_point()
```
...
6. Suppose that your prior for the proportion of houses that have at least one garage is Beta(9, 1).  What is your posterior? Assume a beta-binomial model for this proportion. [D]
<ol>
<li> Beta(954, 46)
<li> Beta(963, 46)
<li> Beta(954, 47)
...
7. Which of the following statements is true about the dataset? [C]
<ol>
...
8. Test, at the $\alpha = 0.05$ level, whether homes with a garage have larger square footage than those without a garage. [A]
<ol>
...
10. When regressing $\log$(`price`) on $\log$(`area`), there are some outliers.  Which of the following do the three most outlying points have in common? [D]
<ol> 
...
12. How many neighborhoods consist of only single-family homes? (e.g. `Bldg.Type` = `1Fam`) [D]
<ol> 
<li> 0
<li> 1
<li> 2 
...
13. Using color, different plotting symbols, conditioning plots, etc., does there appear to be an association between $\log$(`area`) and the number of bedrooms above ground (`Bedroom.AbvGr`)? [A]
<ol> 
<li> Yes
<li> No
</ol>
...
14. Of the people who have unfinished basements, what is the average square footage of the unfinished basement? [B]
<ol> 
<li> 590.36
<li> 595.27
<li> 614.37
